...
import com.microsoft.docs.iothub.samples.db.IotEntityConfig;
...
public class ReadDeviceToCloudMessages {
	...
	private IotEntityConfig iotEntityConfig = new IotEntityConfig();
  ...
  private static final String EVENT_HUBS_COMPATIBLE_ENDPOINT = "sb://iothub-ns-tmpoc-7203428-c1ced5b71a.servicebus.windows.net/";

  // az iot hub show --query properties.eventHubEndpoints.events.path --name {your IoT Hub name}
  private static final String EVENT_HUBS_COMPATIBLE_PATH = "tmpoc";

  // az iot hub policy show --name service --query primaryKey --hub-name {your IoT Hub name}
  private static final String IOT_HUB_SAS_KEY = "90AkpBFGsnavw6y4HCpLhYuA1VKo9wd3lj2JXynnqRA=";
  ...
  public static void main(String[] args) throws Exception {
    ...
    ReadDeviceToCloudMessages readdevicesToCloudObj = new ReadDeviceToCloudMessages();
    ...
    try (EventHubConsumerAsyncClient eventHubConsumerAsyncClient = eventHubClientBuilder.buildAsyncConsumerClient()) {
    	...
    	readdevicesToCloudObj.receiveFromAllPartitions(eventHubConsumerAsyncClient);
  ...
  private  void receiveFromAllPartitions(EventHubConsumerAsyncClient eventHubConsumerAsyncClient) {
    ...
    eventHubConsumerAsyncClient
        ...
        .subscribe(partitionEvent -> {
          ...
          String telemetricData =  partitionEvent.getData().getBodyAsString();
          message.setTelemetricData(telemetricData);
          
          System.out.printf("%nTelemetry received from partition %s:%n%s",
              partitionEvent.getPartitionContext().getPartitionId(), telemetricData);
          
          System.out.printf("%nApplication properties (set by device):%n%s", partitionEvent.getData().getProperties());
          Map<String, Object> map = partitionEvent.getData().getSystemProperties();
           message.setMessageId((String)map.get("message-id"));
           message.setDeviceId((String)map.get("iothub-connection-device-id"));
           message.setSource((String)map.get("iothub-message-source"));
           
          System.out.printf("%nSystem properties (set by IoT Hub):%n%s",
        		  map);
          iotEntityConfig.insertWithQuery(message);
